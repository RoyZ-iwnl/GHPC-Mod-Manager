...
public interface IModManagerService
{
    ...
    Task<List<GitHubRelease>> GetModReleasesAsync(string modId, bool forceRefresh = false);
...
public partial class ModManagerService : IModManagerService
{
    ...
    /// </summary>
    public async Task<List<GitHubRelease>> GetModReleasesAsync(string modId, bool forceRefresh = false)
    {
        try
        {
            // 从已缓存的mod列表中找到对应配置
            ...
            if (IsGitHubApiUrl(modConfig.ReleaseUrl))
            {
                ...
                return await _networkService.GetGitHubReleasesAsync(owner, repo, forceRefresh);
            }
            else
            {
